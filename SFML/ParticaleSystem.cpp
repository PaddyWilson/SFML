...
{
	// give a random velocity and lifetime to the particle ((float)rand()/(float)(RAND_MAX)) * a)
	float angle = ((std::rand() % 360) * 3.14f / 180.f) + 0;// +0;
	float speed = (std::rand() % 150) + 150.f;
	//std::cout << angle << " " << speed << " " << std::cos(angle) * speed << " " << std::sin(angle) * speed << " " << std::endl;
	//printf("%f\n", angle);
